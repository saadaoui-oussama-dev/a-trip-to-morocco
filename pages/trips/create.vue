...
div
  ...
  div.text-xl banner :
  input(type="file" ref="input-banner store-tour.banner" @change="readImage")
  div.prev(ref="prev-banner insert-banner" @click="insertImage")
  div.text-xl images :
  .images.flex.flex-wrap
    .flex.flex-col(v-for="(num, index) in [0,1,2,3,4,5,6,7]" :key="index + 'image'")
      input(type="file", :ref="`input-image${index} store-tour.images[${index}]`", @change="readImage")
      div.prev(:ref="`prev-image${index} insert-image${index}`", @click="insertImage")
  div.text-xl days :
    div(v-for="(day, index) in tour.days" :key="index + 'day'")
      ...
      input(type="file", :ref="`input-day${index} store-tour.days[${index}].image`", @change="readImage")
      div.prev(:ref="`prev-day${index} insert-day${index}`", @click="insertImage")
  button.bg-heath.text-white.text-lg.mr-12(@click="createTour('p')") privateTripCreate
  button.bg-teal.text-white.text-lg.mr-12(@click="createTour('d')") dayTripCreate
  button.bg-kashmir.text-white.text-lg.mr-12(@click="createTour('a')") activityCreate
</template>
...
import mediaReader from '~/plugins/mediaReader'
...
export default {
  ...
  methods: {
    async createTour(type) {
      ...
      const res = await this.$apollo.provider.defaultClient.mutate({
    ...
    readImage(e) {
      mediaReader.image(e.target, this)
    },
    insertImage(e) {
      mediaReader.insert(e.target, this)
    },
  },
}
</script>
...
.prev {
  @apply w-32 h-32 bg-center bg-cover border-2 border-solid border-black cursor-pointer;
}
</style>
